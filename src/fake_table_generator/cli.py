from generation import TargetOuput, PostgresqlType, main
...
@click.command()
@click.option('--target', type=click.Choice([el.name for el in TargetOuput]), default = TargetOuput.csv.name)
@click.option('--nb_tables', type=int, required=True)
...
def generate(target, nb_tables, nb_min_cols, nb_max_cols, nb_min_rows, nb_max_rows, available_types, languages):
    main(TargetOuput[target], nb_tables, nb_min_cols, nb_max_cols, nb_min_rows, nb_max_rows, [PostgresqlType(el) for el in available_types], languages)
