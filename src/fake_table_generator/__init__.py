...
@click.option(
    "--target",
    type=click.Choice([el.name for el in TargetOutput]),
    default=TargetOutput.csv.name,
)
...
@click.option(
    "--available_types",
    type=click.Choice([el.name for el in PostgresqlType]),
    multiple=True,
...
def main(
    ...
    generate(
        ...
        nb_min_rows,
        nb_max_rows,
        [PostgresqlType(el) for el in available_types],
        languages,
    )
